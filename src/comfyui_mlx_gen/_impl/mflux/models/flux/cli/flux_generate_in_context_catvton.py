...
def main():
    ...
    try:
        for seed in args.seed:
            ...
            image.get_right_half().save(path=output_path, export_json_metadata=args.metadata, overwrite=args.replace)
            if args.save_full_image:
                image.save(path=output_path.with_stem(output_path.stem + "_full"), overwrite=args.replace)
